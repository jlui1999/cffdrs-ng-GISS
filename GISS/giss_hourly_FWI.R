...
numargs <- length(args)
if (numargs < 2) {
  print('Usage: Rscript giss_hourly_FWI.R  <data file> <output file> [ffmc] [dmc] [dc]')
  quit()
}
...
if (if.initializeCodes) {
  ...
  data_fwi <- hFWI(data, ffmc_old=ffmc, dmc_old=dmc, dc_old=dc)
} else {
  data_fwi <- hFWI(data)
}
...
# file).
write.csv(data_fwi, outputfile, row.names = FALSE)
